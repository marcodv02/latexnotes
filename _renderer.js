...
async function getPath(pathType) {
  try {
    const path = await ipc.invoke('get-path', pathType);
    return path;
  } catch (error) {
    throw new Error(`Errore nel richiedere il percorso: ${error.message}`);
  }
}
let settingsDir= '/home/marco/.config/test3';
getPath("userData").then(el => {
	settingsDir= el;
	console.log(settingsDir);
});
//const settingsDir = await getPath("userData");//remote.app.getPath("userData");
const themesDir = path.join(settingsDir, "assets/themes");
console.log(themesDir);
const keyboardsDir = path.join(settingsDir, "keyboards");
const fontsDir = path.join(settingsDir, "assets/fonts");
...
ipc.once("getThemeOverride", (e, theme) => {
    if (theme !== null) {
        ...
        _loadTheme(require(path.join(themesDir, 'tron')));//window.settings.theme+".json")));
    }
});
...
async function initUI() {
    ...
    <section class="mod_column" id="mod_column_right">
        <h3 class="title"><p>PANEL</p><p>GRAPH</p></h3>
    </section>`;
    ...
    // Left column
    window.mods.clock = new Clock("mod_column_right");
	window.mods.graph = new Graph("mod_column_right");
	//was mod_column_left
    ...
    shellContainer.innerHTML += `
        <ul id="main_shell_tabs">
			...
			<li id="pdf_tab" onclick="window.pdf(0);"><p>PDF reader</p></li>
    ...
    window.term[0].term.writeln("\033[1m"+`Welcome to eDEX-UI CUSTOM `+"\033[0m");//"\033[1m"+`Welcome to eDEX-UI v${electron.remote.app.getVersion()} - Electron v${process.versions.electron}`+"\033[0m");
        ...
        ipc.once("ttyspawn-reply", (e, r) => {
            if (r.startsWith("ERROR")) {
                ...
                window.term[number].onclose = e => {
					...
					console.log(number, window.term[number]);
...
window.pdf = abs_number => {
	console.log(abs_number);
	console.log("Conf number");

	$.ajax({
		url: "http://localhost/test3/Api/list_all.php"
	}).done(function (e){
		console.log(e);
		if (e.status==2) {
			for (let i= 0; i<10; i++)
